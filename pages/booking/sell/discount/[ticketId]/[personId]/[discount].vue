  ...
  <UCard class="w-full" :ui="{
    ...
    <template #header>
      <div class="mx-auto p-4 text-lg text-center justify-center grid grid-cols-1 place-items-center">
        ...
        <div >
          Cоздать новый билет на основе указанного, указав скидку в заданное число %, и, одновременно, увеличив цену билета на ту же самую сумму.
        </div>
...
onMounted(() => {
  ...
  const { ticketId: tId, personId: pId, discount: disc } = router.currentRoute.value.params;


  if (!isNaN(tId) && !isNaN(pId) && !isNaN(disc)) {

    ticketId.value = parseInt(tId);
    personId.value = parseInt(pId);
    discount.value = parseFloat(disc);

    if (discount.value > 100 || discount.value < 1) {
      errorHappened.value = true
      errorMsg.value = "Недопустимый путь! Поле discount должно быть от 1 до 100"
    }

  } else {
    // Redirect to a different route if any parameter is not a number
    errorHappened.value = true
    errorMsg.value = "Недопустимый путь! Валидный путь: /booking/sell/discount/[число]/[число]/[число]"
  }
})
...
const getDiscount = async () => {
    ...
    if (ticketError.value?.data) {
      ...
      successMsg.value = "Билет с индивидуальной скидкой создан!"
    }
